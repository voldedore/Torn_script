...
// @version      1.3.0
// @updateURL    https://github.com/voldedore/Torn_script/raw/main/%5BV.O.T.T%5D%20Employee%20Addiction.user.js
// @downloadURL  https://github.com/voldedore/Torn_script/raw/main/%5BV.O.T.T%5D%20Employee%20Addiction.user.js
...
(async function () {
    ...
    - Fix the dynamic className generated by Torn's front-end library.
    - Change some console output to debug.
    */
    ...
    if (storedApiKey !== 'null') {
        try {
            ...
            const tornDelimiter = document.querySelector('[class^="delimiter___"]');

            // Append the delimiter as a child of the employBox
            employBox.appendChild(tornDelimiter);
            ...
            allEmployeeIds.forEach(employeeId => {
                ...
                */
                ...
                console.debug(`${name}'s addiction value: ${addiction}, relative value: ${relative}`);
            });

            // ... (your existing code)

            // Log the final dictionary
            console.debug('Employ Dictionary:', Employ);

            // Append the employBox as a child of the element with class 'content___XXX'
            // (we want the 2nd child since this one is nested in another one with the similar class .content_YYY)
            document.querySelectorAll('[class^="content___"]')[1].appendChild(employBox);
        } catch (error) {
            console.error('Error fetching data:', error.message);
        }
    } else {
        console.warn('User has chosen not to provide an API key or entered an incorrect key.');
